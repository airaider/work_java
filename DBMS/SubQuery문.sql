...
from emp e; -- 두개의 서브쿼리가 각각 수행되므로 성능이 안좋다 => join이 더 효율적
...
from emp, salgrade
where job = 'MANAGER' and sal between losal and hisal and sal >(select avg(sal) from emp);
...
-- 부서별 최저 급여를 받는 사원의 정보 조회 
 ...
 select *
 from emp e, (select deptno, min(sal) as deptSal from emp group by deptno) d
...
select *
from goods g, (select distinct gno from orders) o
where g.gno=o.gno;
