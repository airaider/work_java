...
select if(grouping(deptno)=1,'totalsal' ,ifnull(deptno,'프리랜서')) as 'deptno', round(avg(sal)) as avgsal
from emp
...
select * from board;
select b.no as no, id, title, date_format(regdate, '%y-%m-%d') as regdate, contents, rfilename, sfilename
from board b
...
select *
from emp
where mgr = (select empno from emp where ename='SCOTT') or mgr = (select empno from emp where ename='CLARK');
...
-- 상호 연관 sub query를 from절의 sub query로 변환
select empno, ename, job, sal, deptno
from emp, (select avg(sal) as avgsal, deptno as avgdeptno
			 from emp 
             group by deptno) d
...
select * from goods g
where exists (select 1 from orders o where o.gno = g.gno);
...
values( DATE_SUB(SYSDATE(), INTERVAL 3 DAY ), 'ssafy', 1 , 40 ,'서울');
insert into orders (odate, id, gno, quantity, address)
values( DATE_SUB(SYSDATE(),INTERVAL 2 DAY ), 'jaen', 4 , 2 ,'서울');
insert into orders (odate, id, gno, quantity, address)
values( DATE_SUB(SYSDATE(),INTERVAL 2 DAY), 'jaen', 2 , 2 ,'서울');
insert into orders (odate, id, gno, quantity, address)
values( DATE_SUB(SYSDATE(),INTERVAL 1 DAY), 'ssafy', 1 , 1 ,'서울');
insert into orders (odate, id, gno, quantity, address)
values( DATE_SUB(SYSDATE(), INTERVAL 1 DAY), 'ssafy', 2 , 2 ,'서울');
insert into orders (odate, id, gno, quantity, address)
...
select gno, sum(quantity) as amount
from orders
...
select o.gno, g.brand, g.price, sum(quantity) as amount
...
from goods
join(select gno, sum(quantity) as amount
from orders
group by gno
order by amount desc
limit 0,2) a
...
-- 제품의 총 판매 금액이 높은 상품 3개를 조회 하시오 
select gno, brand, amount, price, amount*price as total
from goods 
join (select gno, sum(quantity) amount
			  from orders
			  group by gno) o
using (gno)
order by total desc
...
select gno, cno, amount from goods
join (select gno, sum(quantity) amount from orders group by gno) o
using (gno)
group by cno
order by amount desc;

select cno, name, ifnull(amount, 0) as amount
	   ,ifnull(totalPrice, 0) as totalPrice
from category
left join(select cno, sum(quantity) as amount, sum(total) as totalPrice
		from (select gno, cno, quantity, quantity * price as total
			from orders
			join goods
			using (gno)) a
		group by cno) b
