...
-- vender 전용
select odate, ono, o.gno as gno, brand, id, price, quantity, price*quantity as total_price
from goods g, orders o
where g.gno=o.gno;

-- ansi 
select odate, ono, gno, brand, id, price, quantity, price*quantity as total_price
from goods 
join orders 
using (gno);
...
select odate, ono, o.gno as gno, brand, id, price, quantity, price*quantity as total_price
from goods g, orders o
where g.gno = o.gno and date_format(odate,'%Y%m') = date_format(sysdate(),'%Y%m');
-- where g.gno=o.gno and year(odate) = year(sysdate()) and month(odate) = month(sysdate());
...
-- vender 
select ono, odate, brand, name, price, quantity, price*quantity as total_price
...
-- vender 
select g.gno, brand, sum(quantity) as total_quantity
from goods g, orders o
where g.gno=o.gno
group by gno
order by gno;
